...
stage('Deploy') {
            sshagent(credentials: ['flask-ec2-server']) {
                sh(script: "ssh -o StrictHostKeyChecking=no ubuntu@3.38.237.141")
                sh(script:"ssh ubuntu@3.38.237.141 'sudo docker ps -q --filter name=docker-flask | grep -q . && sudo docker rm -f \$(sudo docker ps -aq --filter name=docker-flask)'")
